...
func New(apiKey, model string) *Tagger {
	...
	if apiKey != "" {
		t.client = openai.NewClient(apiKey)
		t.enabled = true
	}
...
- Weights must be between 0.3 and 1.0.`
	...
	resp, err := t.client.CreateChatCompletion(limitedCtx, request)
	if err != nil {
		return nil, err
	}
...
func (t *Tagger) shouldDisableAI(err error) bool {
	...
	if errors.As(err, &apiErr) {
		...
		if strings.Contains(message, "status code: 429") || strings.Contains(message, "exceeded your current quota") {
			t.enabled = false
			return true
		}
	}
